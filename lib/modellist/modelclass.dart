...
class Quotes{
  ...
  Quotes({required this.name,required this.quotes,required this.birthday,required this.dream,required this.failure,required this.fun,required this.gaming,required this.general,required this.humorous,required this.life,required this.motivational,required this.programming,required this.success,required this.travel});

factory Quotes.from(Map m1)
{
  return Quotes(name: m1['author'], quotes: m1['quotes'], birthday: List.from(m1['birthday']), dream: List.from(m1['dream']), failure: List.from(m1['failure']), fun: List.from(m1['fun']), gaming: List.from(m1['gaming']), general: List.from(m1['general']), humorous: List.from(m1['humorous']), life: List.from(m1['life']), motivational: List.from(m1['motivational']), programming: List.from(m1['programming']), success: List.from(m1['success']), travel: List.from(m1['travel']));
}
}
